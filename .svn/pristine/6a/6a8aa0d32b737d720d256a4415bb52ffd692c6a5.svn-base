...
public class Network {
	...
	public static int HEIGHT_MATRIX; //alto (nº de inputs)
	public static int WIDTH_MATRIX; //ancho (tamaño inputs)
	public static final double BETA = 0.2; //parámetro para evitar oscilaciones 
	...
	private Matrix previousMatrixWeight; //La necesitamos para el evitar la convergencia y los minimos locales (añadimos el beta)
	...
	private ArrayList<BigDecimal> errorTraining; //Almacena el error ponderado de cada iteración
...
//	}
	...
	public void setUpNetwork (BigDecimal errorCuote, int numPatrones, int neuronSize, boolean bias, int iterMax){
		...
		int inicio = 1; //desde donde comienza a seleccionar los patrones de aprend (podremos poner fechas más adelante)
			//EL tamaño de la neurona no puede verse involucrado en la OBTENCIÓN de los datos
	...
	public void trainingAvoidingOscillation (){
		...
		while (!endAlgorithm && iteration < ITERATION_MAX){  
			...
			for (int i = 0; i<inputs.size(); i++){
				...
				///INCLUIR VARIACIÓN DELTA PARA EVITAR OSCILACIONES--> BETA (W(t) - W(t-1)		
				//Antes de la actualización de la matriz de pesos, guardamos la actual como la previa para la siguiente iteracción
				...
				matrixWeight = Matrix.addition(matrixWeight, aux2); //Variación beta añadida
